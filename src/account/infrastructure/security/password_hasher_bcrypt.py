...
from account.domain.account.ports import PasswordHasher
...
class BcryptPasswordHasher(PasswordHasher):
    ...
    def hash_sync(self, raw_password: RawPassword) -> AccountPasswordHash:
        log.debug("hash")
        base64_hmac_peppered = self._add_pepper(raw_password, self._pepper)
        salt = bcrypt.gensalt(rounds=self._work_factor)
        return AccountPasswordHash(bcrypt.hashpw(base64_hmac_peppered, salt))

    def verify_sync(
        self, raw_password: RawPassword, hashed_password: AccountPasswordHash
    ) -> bool:
        log.debug("verify")
        base64_hmac_peppered = self._add_pepper(raw_password, self._pepper)
