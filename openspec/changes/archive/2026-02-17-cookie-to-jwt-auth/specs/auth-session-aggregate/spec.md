...
**Reason**: Replaced by `RefreshToken` entity at `domain/refresh_token/entity.py`. Server-side sessions are eliminated in favor of stateless JWT access tokens + server-side refresh tokens.
...
**Reason**: Replaced by `RefreshTokenRepository` protocol at `domain/refresh_token/repository.py`.
...
**Reason**: Replaced by `RefreshTokenNotFoundError` and `RefreshTokenExpiredError` at `domain/refresh_token/errors.py`.
